...
summary(model.dlm2)
```
Dari hasil tersebut terdapat beberapa peubah yang berpengaruh signifikan terhadap taraf nyata 5% yaitu xt, xt-7, xt-10. Adapun keseluruhan model yang terbentuk adalah
$$\hat{Y} = 42.535 --23.945Xt+ 14.530Xt-1 + ... -21.183Xt-7 + 3.101Xt-8...$$
...
GoF(model.dlm2)
```
Model tersebut merupakan model yang sangat baik dengan nilai MAPE yang hanya 0.03% kurang dari 10%.
...
Hasil di atas menunjukkan bahwa selain peubah xt−1, hasil uji t menunjukkan nilai-p pada peubah ≥0.05 Hal ini menunjukkan bahwa yt−1 berpengaruh signifikan terhadap yt. Model keseluruhannya adalah sebagai berikut:
$$\hat{Y} = 3.8364 +0.0725Xt -0.4271X(t-1) + 0.88537Y(t-1) $$
...
```
Dari tabel di atas, dapat terlihat bahwa nilai AIC terendah didapat ketika p=15 dan q=5, yaitu sebesar -20,56587. Artinya, model autoregressive optimum didapat ketika p=6 dan q=1
Selanjutnya dapat dilakukan pemodelan dengan nilai p dan q optimum seperti inisialisasi di langkah sebelumnya.
## Pemodelan Autoregressive lag optimum
```{r}
...
BIC(model.ardl.opt2)
```
Hasil di atas menunjukkan bahwa selain peubah xt−1, hasil uji t menunjukkan nilai-p pada peubah ≥0.05 Hal ini menunjukkan bahwa xt berpengaruh signifikan terhadap yt. Model keseluruhannya adalah sebagai berikut:

$$\hat{Y} = 10.8906-11.1056Xt +...-0.3796Y(t-5) $$
```{r}
...
GoF(model.ardl.opt2)
```
Berdasarkan akurasi di atas, terlihat bahwa nilai MAPE sangat kecil. Jauh di bawah 10%
...
```
Model 1 : p-value 0.31055 (< 0.05). Artinya, penambahan lag dari Y (L(Yt)) memberikan peningkatan signifikan.
Dengan demikian, Model 1 tidak cukup memadai.

Model 2 : p-value = 0.00505 (> 0.05).
Artinya, penambahan lag dari Y (L(Yt)) tidak memberikan peningkatan signifikan.
